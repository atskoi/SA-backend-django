...
from pac.models import (Country, CountryHistory, Province,
...
class LinehaulLaneRoutesDetailPanelSerializer(serializers.ModelSerializer):
    ...
    lane_route_history = serializers.SerializerMethodField()
    ...
    def get_lane_route_history(self, obj):
        lane_route_history = LaneRouteHistory.objects.filter(
            lane_route=obj)
        return LaneRouteHistorySerializer(lane_route_history, many=True).data
    ...
    class Meta:
        ...
        fields = ['lane_route_id', 'origin_terminal_id', 'origin_terminal_code', 'destination_terminal_id',
                  ...
                  'updated_on', 'lane_route_history', 'is_active']
...
class LaneRouteHistorySerializer(serializers.ModelSerializer):
    class Meta:
        model = LaneRouteHistory
        fields = ['lane_route_version_id', 'route_legs',
                  'version_num', 'is_latest_version', 'is_active', 'updated_on']
...
class GriReviewSerializer(serializers.ModelSerializer):
    service_level_name = serializers.CharField(source='service_level.service_level_name', read_only=True)
    class Meta:
        model = GriReview
        fields = "__all__"
