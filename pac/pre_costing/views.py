...
from pac.pre_costing.serializers import (
    ...
    BrokerContractDetailPanelSerializer,
...
class DockCostsDetailPanelView(generics.RetrieveAPIView):
    ...
    queryset = TerminalCost.objects.filter()
    lookup_field = "terminal_cost_id"
...
class city_search_by_name_pyodbc(views.APIView):
    # used by the RequestInfo tab to lookup a ServicePoint as a "city"
    def get(self, request, *args, **kwargs):
        city_name = kwargs.get("city_name")
        province_id = kwargs.get("province_id")
        conn = pyodbc_connection()
        cursor = conn.cursor()
        query = f"""SELECT TOP 25 lt.ID ServicePointID, lt.Name ServicePointName
            FROM dbo.V_LocationTree lt
            WHERE lt.ProvinceID = {province_id} AND lt.PointTypeID = 6
                                   AND lt.Name LIKE '{city_name}%'
                               ORDER BY Name"""
