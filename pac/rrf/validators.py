...
from pac.rrf.models import(PointType, RequestSectionLanePointType, RequestSection, RequestSectionLane, RequestSectionLanePointType,
...
# def check_lane_service_matrix(lane, **kwargs):
#     pass
...
def resolve_group_id_from_code(lane, code, is_origin):

    if is_origin:
        name_attribute = "origin_group_type_name"
        id_attribute = "origin_group_id"
    else:
        name_attribute = "destination_group_type_name"
        id_attribute = "destination_group_id"

    if getattr(lane, name_attribute) == "Country":
        setattr(lane, id_attribute, Country.objects.filter(country_code=code)[0].country_id)
    elif getattr(lane, name_attribute)== "Province":
        setattr(lane, id_attribute, Province.objects.filter(province_code=code)[0].province_id)
    elif getattr(lane, name_attribute) == "Region":
        setattr(lane, id_attribute, Region.objects.filter(country_code=code)[0].region_id)

def resolve_point_id_from_code(lane, code, is_origin):
    # logging.info(f'code: {code}')
    try:
        if is_origin:
            name_attribute = "origin_point_type_name"
            id_attribute = "origin_point_id"
        else:
            name_attribute = "destination_point_type_name"
            id_attribute = "destination_point_id"

        if getattr(lane, name_attribute) == "Country":
            setattr(lane, id_attribute, Country.objects.filter(country_code=code)[0].country_id)
        elif getattr(lane, name_attribute) == "Region":
            setattr(lane, id_attribute, Region.objects.filter(region_code=code)[0].region_id)
        elif getattr(lane, name_attribute) == "Province":
            setattr(lane, id_attribute, Province.objects.filter(province_code=code)[0].province_id)
        elif getattr(lane, name_attribute) == "Terminal":
            setattr(lane, id_attribute, Terminal.objects.filter(terminal_code=code)[0].terminal_id)
        elif getattr(lane, name_attribute) == "Basing Point":
            setattr(lane, id_attribute, BasingPoint.objects.filter(basing_point_name=code)[0].basing_point_id)
        elif getattr(lane, name_attribute) == "Postal Code":
            setattr(lane, id_attribute, PostalCode.objects.filter(postal_code_name=code)[0].postal_code_id)
        elif getattr(lane, name_attribute) == "Service Point":
            setattr(lane, id_attribute, ServicePoint.objects.filter(service_point_name=code)[0].service_point_id)
    except Exception as e:
        lane.uni_status = "INVALID"
        lane.status_message = {"L999": "General Validation Exception"}


def resolve_lane_ids_orm(lane):
    lane.orig_group_type_id = RequestSectionLanePointType.objects.filter(is_group_type=True, 
    request_section_lane_point_type_name=lane.origin_group_type_name)[0].request_section_lane_point_type_id

    lane.origin_point_type_id = PointType.objects.filter(point_type_name=lane.origin_point_type_name)[0].point_type_id

    resolve_group_id_from_code(lane, lane.origin_group_code, True)
    resolve_point_id_from_code(lane, lane.origin_point_code, True)

    lane.destination_group_type_id = RequestSectionLanePointType.objects.filter(is_group_type=True, 
    request_section_lane_point_type_name=lane.destination_group_type_name)[0].request_section_lane_point_type_id

    lane.destination_point_type_id = PointType.objects.filter(point_type_name=lane.destination_point_type_name)[0].point_type_id

    resolve_group_id_from_code(lane, lane.destination_group_code, False)
    resolve_point_id_from_code(lane, lane.destination_point_code, False)

def resolve_pricing_point_ids(pricing_point):
    pricing_point.origin_post_code_id = PostalCode.objects.filter(
        postal_code_name=pricing_point.origin_postal_code_name)[0].postal_code_id

    pricing_point.destination_post_code_id = PostalCode.objects.filter(
        postal_code_name=pricing_point.destination_postal_code_name)[0].postal_code_id
