...
class RequestFilteredView(AppView):
    ...
    def prepare_filter(self, data, kwargs):
        ...
        self.GET_FILTERED_QUERY = self.GET_FILTERED_QUERY.format(
            ...
            type_filter=""" AND (R.UniType IS NULL OR R.UniType IN ('Non-Customer Tariff','Customer Tariff','Other Tariff', 'Tender Tariff', 'Annual Review Tariff'))""",
            user_id=self.request.user.user_id
        )
        return data


class ReviewFilteredView(AppView):
    PRIMARY_TABLE = 'dbo.Request'
    PRIMARY_KEY = 'r.RequestID'
    COLUMN_MAPPING = dQueries.ColumnMapping
    schema = DashboardRequestSchema
    ...
    def prepare_filter(self, data, kwargs):
        ...
        self.GET_FILTERED_QUERY = self.GET_FILTERED_QUERY.format(
            service_offering_id=service_offering_id,
            type_filter=""" AND (R.UniType IS NULL OR R.UniType IN ('GRI Review Tariff', 'Annual Review Tariff'))""",
            user_id=self.request.user.user_id
        )
        return data
