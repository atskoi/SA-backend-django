ColumnMapping = {
    ...
    "pricing_analyst_name": {"sortColumn": 'ISNULL(UPA.UserName,'')', "filter": " AND r.PricingAnalystID IN ({0}) "},
    ...
    "request_status_type_name": {"sortColumn": 'RST.RequestStatusTypeName', "filter": " AND r.RequestStatusTypeID IN ({0}) "},
    "sales_representative_name": {"sortColumn": 'ISNULL(USR.UserName,'')', "filter": " AND r.SalesRepresentativeID IN ({0}) "},
...
WHERE rst.RequestStatusTypeName NOT IN ('Published', 'Archived') AND r.IsActive = 1 AND r.IsInactiveViewable = 1 AND (
r.CurrentEditorID = @UserID OR
r.CurrentEditorID IN (SELECT UserID FROM dbo.[User] WHERE UserManagerID = @UserID)
)),

A3 AS (
SELECT r.RequestID, r.CurrentEditorID UserID, r.PricingAssigned AssignedOn, null CompletedOn, p.PersonaName AssignedPersona
FROM dbo.Request r
...
WHERE r.RequestID IN (SELECT RequestID FROM A1) AND r.IsActive = 1 AND r.IsInactiveViewable = 1
AND p.PersonaName IN ('Pricing Analyst', 'Credit Analyst', 'Credit Manager', 'Partner Carrier')
),
B1 AS (
SELECT DISTINCT r.RequestID, r.RequestStatusTypeID, r.PricingAssigned AssignedOn, U.UserName, u.UserID
FROM dbo.Request r
INNER JOIN dbo.[User] U ON r.CurrentEditorID = U.UserID
WHERE r.RequestID IN (SELECT RequestID FROM A1) AND r.IsActive = 1 AND r.IsInactiveViewable = 1
),
C1 AS (
...
SELECT R.RequestID,
    ...
    R.RequestSourceID as request_source_id,
    ...
    ISNULL(r.PricingAnalystID,'') AS pricing_analyst_id,
    ISNULL(UPA.UserName,'') AS pricing_analyst_name,
    ISNULL(r.SalesRepresentativeID,'') AS sales_representative_id,
    ISNULL(USR.UserName,'') AS sales_representative_name,
    ISNULL(r.CurrentEditorID,'') AS current_editor_id,
    ISNULL(CUE.UserName,'') AS current_editor_name,
    ISNULL(I.[Priority],'') AS [priority],
    r.RequestStatusTypeID AS request_status_type_id,
    ...
    CASE WHEN (A.AccountID IS NULL OR A.AccountNumber = '0') THEN 'New' ELSE 'Expanded' END AS business_type_name,
    ...
    CASE WHEN r.CurrentEditorID = @UserID THEN 1 ELSE 0 END AS is_current_editor,
...
LEFT JOIN dbo.[User] UPA ON r.PricingAnalystID = UPA.UserID
LEFT JOIN dbo.[User] USR ON r.SalesRepresentativeID = USR.UserID
LEFT JOIN dbo.[User] CUE ON r.CurrentEditorID = CUE.UserID
