...
from pac.rrf.models import (LastAssignedUser, RequestEditorRight,
...
#                     completed_on=current_time if self.next_request_status_type.is_final else None)
        ...
        if self.next_request_status_type.request_status_type_name in self.request_status_type_system_calculators:
            ...
            logging.info(f'created manager {new_workflow_manager} ')
    ...
    def _rrf_submitted_system_calculator(self):
        if not self.request.request_information.customer.account: # assign to a credit analyst if no customer or account
            return RequestStatusType.objects.filter(request_status_type_name="Credit Analysis").first()

    def _assign_credit_analyst_system_calculator(self):
        return RequestStatusType.objects.filter(request_status_type_name="Credit Analysis").first()

    def _credit_approved_system_calculator(self):
        return RequestStatusType.objects.filter(request_status_type_name="Cost+ Analysis").first()

    def _cost_plus_approved_system_calculator(self):
        return RequestStatusType.objects.filter(request_status_type_name="With Pricing").first()
...
def request_editor_right_update(action, request_status_instance, new_current_editor_instance):
    ...
    if request_type_name == "Cost+ Analysis":
