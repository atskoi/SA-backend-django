...
from pac.rrf.models import ImportFile, RequestSectionLaneImportQueue, RequestSectionLanePricingPointImportQueue, \
    RequestSectionLanePointType, RequestSectionLane, RequestSection
...
def validate_lanes(file):
    ...
    for lane in lanes:
        if do_validate_lanes(file, header, lane, location_hierarchy, lanes=lanes, existing_request_section_lanes=existing_request_section_lanes) == 'VALID':
            ...
            request_section = RequestSection.objects.get(pk=file.request_section_id)
            sub_service_level_id = request_section.sub_service_level.sub_service_level_id
            # Validate against service matrix
            # Should be able to set lane.uni_status here
            lane.save()
    ...
    file.save(update_fields=["duplicate_lane_count", "directional_lane_count", "between_lane_count", 'flagged_count', 'unserviceable_count'])
