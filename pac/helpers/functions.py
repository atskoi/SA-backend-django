...
def update_instance_history_bulk(instances_all, globals, batch_size=0, update_fields=None):
    ...
    logging.info(f'about to save updated records {instances_all}')
    return []

    instance_chunks = [instances_all[i * batch_size: (i + 1) * batch_size]
...
def create_instance_history(sender, instance, globals):
    try:
        ...
        primary_id = instance
        # TODO: without the request context, how can we get the calling user's name?
        print(f'INSTANCE HISTORY table {table_name} id {primary_id}')
        audit_statement = '''EXEC dbo.[Audit_Record] @TableName = '{table}', @PrimaryKeyValue = {pk_id}, @UpdatedBy = '{user_name}' '''
        audit_statement = audit_statement.format(table = table_name, pk_id = primary_id, user_name = 'Default Auditing')
        conn = pyodbc_connection()
        print(f'before execute {audit_statement}')
#         conn.execute(audit_statement)
#         conn.commit()
        print(f'after audit')
    except oops as ex:
        print(f'Audit exception {oops} or {ex}')
        # attempt legacy auditing from django models
