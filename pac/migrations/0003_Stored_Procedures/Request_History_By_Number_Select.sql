...
SELECT U.UserVersionID,
	U.UserName,
    (SELECT um.UserName FROM dbo.[User_History] um
    WHERE u.UserManagerVersionID = UM.UserVersionID) AS UserManagerName
...
(
	...
	LEFT JOIN UT I ON R.InitiatedByVersionID = I.UserVersionID
	LEFT JOIN dbo.RequestInformation_History RIC ON R.RequestVersionID = RIC.RequestVersionID
	...
	LEFT JOIN UT AS S ON r.SalesRepresentativeVersionID = S.USerVersionID
	LEFT JOIN UT AS E ON r.CurrentEditorVersionID = E.UserVersionID
	...
	LEFT JOIN UT B ON R.SubmittedByVersionID = B.USerVersionID
