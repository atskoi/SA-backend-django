...
def buildUpdateSchema (columns):
    ...
    for column in columns:
        if column['type'] == 'string':
            tempSchema.get('records').get('items').get('properties').get('data').get('properties')\
            .update({column['fieldName']:  {"anyOf":[{"type": "string"}, {"type": "null"}]} })
        elif column['type'] == 'boolean':
            tempSchema.get('records').get('items').get('properties').get('data').get('properties')\
            .update({column['fieldName']: {"anyOf":[{"type": "boolean"}, {"type": "null"}]} })
        else:
            tempSchema.get('records').get('items').get('properties').get('data').get('properties')\
            .update({column['fieldName']: {"anyOf":[{"type": "number"}, {"type": "null"}]} })
    return tempSchema
...
def buildInsertSchema (columns):
    ...
    for column in columns:
        if column['type'] == 'string':
            tempSchema.get('items').get('properties').get('data').get('properties')\
            .update({column['fieldName']: {"anyOf":[{"type": "string"}, {"type": "null"}]} })
        elif column['type'] == 'boolean':
            tempSchema.get('items').get('properties').get('data').get('properties')\
            .update({column['fieldName']: {"anyOf":[{"type": "boolean"}, {"type": "null"}]} })
